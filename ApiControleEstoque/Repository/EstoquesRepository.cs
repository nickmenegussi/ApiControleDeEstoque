...
namespace ApiControleEstoque.Repository
{
    public class EstoquesRepository
    {
        ...
        public static async Task<List<Estoques>> GetAllEstoquesAsync()
        {
            var query = @"
            SELECT 
                e.IdEstoque,
            ...
            var listEstoques = await connection.QueryAsync<Estoques>(query);
            return listEstoques.AsList();
        }

        public static async Task<Estoques?> GetByIdAsync(long id)
        {
            if (id <= 0) return null;
            var query = @"
            SELECT 
            ...
            using var connection = new SqlConnection(_connectionString);
            return await connection.QueryFirstOrDefaultAsync<Estoques>(query, new { id });
        }

        public static async Task<List<Estoques>> GetEstoquesByTipoEstoqueAsync(string Descricao)
        {
            if (string.IsNullOrWhiteSpace(Descricao)) return new List<Estoques>();
            var query = @"
            SELECT 
                e.IdEstoque, e.IdTipoEstoque, e.Descricao,
                te.Descricao AS DescricaoTiposEstoque
            ...
            using var connection = new SqlConnection(_connectionString);
            var list = await connection.QueryAsync<Estoques>(query, new { Desc = $"%{Descricao}%" });
            return list.AsList();
        }

        public static async Task<List<Estoques>> GetEstoquesByIdTipoEstoqueAsync(long idTipoEstoque)
        {
            if (idTipoEstoque <= 0) return new List<Estoques>();
            var query = @"
            SELECT 
                e.IdEstoque, e.IdTipoEstoque, e.Descricao,
                te.Descricao AS DescricaoTiposEstoque
            ...
            using var connection = new SqlConnection(_connectionString);
            return (await connection.QueryAsync<Estoques>(query, new { IdTipoEstoque = idTipoEstoque })).AsList();
        }
        ...
        public static async Task<List<Estoques>> GetEstoquesByDescricaoAsync(string descricao)
        {
            if (string.IsNullOrWhiteSpace(descricao)) return new List<Estoques>();
            var query = @"
            SELECT 
            ...
            using var connection = new SqlConnection(_connectionString);
            return (await connection.QueryAsync<Estoques>(query, new { Desc = $"%{descricao}%" })).AsList();
        }
        ...
        public static async Task<List<Estoques>> ConsultarIdTipoEstoqueEstoqueTipoEstoqueAsync(PesquisaEstoque vm)
        {
            if (vm == null || string.IsNullOrWhiteSpace(vm.Descricao) || vm.IdTipoEstoque <= 0) 
                return new List<Estoques>();
            var query = @"
            SELECT 
                e.IdEstoque, e.IdTipoEstoque, e.Descricao AS DescricaoEstoque,
                te.Descricao AS DescricaoTiposEstoque
            FROM Estoques e
            INNER JOIN TiposEstoque te ON e.IdTipoEstoque = te.IdTipoEstoque
            ...
            using var connection = new SqlConnection(_connectionString);
            return (await connection.QueryAsync<Estoques>(query, new { IdTipoEstoque = vm.IdTipoEstoque, Desc = $"%{vm.Descricao}%" })).AsList();
        }
